...
class plotOptions(object):
    def __init__(self, xlimL, xlimU, ylimL, ylimU, shield, datatype="AB", autoXlim = False, autoYlim = False):
        """Plot Options
        limits are limits of the plot.

        shield: A Cryo-Shield Object
        ...
        self.ylimL = ylimL
        self.ylimU = ylimU
        self.autoXlim = autoXlim
        self.autoYlim = autoYlim
        ...
        if datatype == "AB":
            ...
            self.xDim = ["Time (s)", "Time (m)", "Time (h)"]
            self.xConversion = np.array([1, 1./60, 1./3600])
            self.yDim = ["Pressure (bar)"]
            self.yConversion = np.array([1])
