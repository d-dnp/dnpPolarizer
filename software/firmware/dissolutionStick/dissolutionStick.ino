...
void updateValves() {
  //this could probably be refactored into a port statement, if speed is ever a concern.
  digitalWrite(valve1, 0b1 & valves);
  //digitalWrite(valve2, 0b1 & ~valves);
  digitalWrite(valve4, 0b1000 & valves);
  digitalWrite(valve8, 0b10000000 & valves);
}
...
void loop() {
  ...
  updateValves();
  //comm.parseRequest();
  comm.processEthernetRequest(&temperature, &pressure, &tSet, &pSet, &regulation, &heaterOn, &valves, &trig);
